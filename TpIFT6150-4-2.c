...
void radon_to_TF2D(float** tf_r, float** tf_i, float** radon_r, float** radon_i) {
    ...
    for(i=0; i<LENGTH/2; i++)
        for(j=0; j<WIDTH; j++) {
            ...
            rayon = (int) round(sqrt(CARRE(x) + CARRE(y)) + WIDTH/2) % WIDTH;

            angle = (int) (NB_PROJECTIONS - round(AngleDeg(x, y, 0, 0, WIDTH, 0))) % NB_PROJECTIONS;
...
int main(char argc, char** argv)
{
    ...
    for(k=0; k<NB_PROJECTIONS; k++) {
        ...
        matrix_rotation_ppv(MatriceImgG, Mat1, k / 360.0 * 2 * PI, LENGTH, WIDTH);
