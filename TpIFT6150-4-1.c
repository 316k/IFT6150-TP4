...
void radon_to_TF2D(float** tf_r, float** tf_i, float** radon_r, float** radon_i) {
    ...
    for(i=0; i<LENGTH/2; i++)
        for(j=0; j<WIDTH; j++) {
            ...
            angle = fmod(NB_PROJECTIONS - round(AngleDeg(x, y, 0, 0, WIDTH, 0)), NB_PROJECTIONS);
            ...
            // Plus proche voisin
            /* tf_r[i][j] = radon_r[(int) angle][(int) rayon]; */
            /* tf_i[i][j] = -radon_i[(int) angle][(int) rayon]; */

            /* tf_r[LENGTH - 1 - i][(WIDTH - j) % WIDTH] = radon_r[(int) angle][(int) rayon]; */
            /* tf_i[LENGTH - 1 - i][(WIDTH - j) % WIDTH] = radon_i[(int) angle][(int) rayon]; */
        }
}
...
int main(char argc, char** argv)
{
    ...
    for(k=0; k<NB_PROJECTIONS; k++) {
        ...
        matrix_rotation_bilin(MatriceImgG, Mat1, k / 360.0 * 2 * PI, LENGTH, WIDTH);
    ...
    // Sauvegarde du spectre de l'image
    /* Recal(MatMFFT, LENGTH, WIDTH); */
    /* Mult(MatMFFT, 60, LENGTH, WIDTH); */
    
    /* SaveImagePgm("fft", MatMFFT, LENGTH, WIDTH); */
